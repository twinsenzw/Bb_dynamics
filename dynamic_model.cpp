...
//***about S vector: it stores 1000 similarity values for each possible nr; in S[0-999][nr-1], but nr-1 in 0-19
{
	...
	{
		int recombination = nr(type, ab[i].type, bb)-1; //recombination is the index used for S, so -1
...
void bind_grow_recombine_BB(vector<BB> &bb, EC ec , vector<AB> ab, parameters p) //binding+growing+recombining bb
{
	...
	for(int i=0;i<bb.size();i++)
	{
		...
		//growth and death are deterministic
		double growth = growth_rate * present_bb[i].n_free;
		double death = p.p0 * present_bb[i].n_bound * ec.concentration ;

		//bind, detach and recombine has to pass min_n filter
		...
		double recombine = p.r * (present_bb[i].n_free+present_bb[i].n_bound); if(recombine <= p.min_n) recombine=0;
		...
		//free cells:
		if(bb[i].n_free!=0)
		bb[i].n_free=bb[i].n_free - bind - recombine + detach + growth;


		//bound cells: 
		if(bb[i].n_bound!=0)
		bb[i].n_bound=bb[i].n_bound - detach - death - recombine + bind;
...
double objective(const vector<double> &present_p, vector<double> &grad, void *data) 
{
	...
	cout<<p.psai<<"\t"<<p.C<<"\t"<<p.x<<endl; // hours-1

	for(int hour=1;hour<=672;hour++)
	{
		...
		bind_grow_recombine_BB(bb, present_ec , present_ab, p);
		grow_EC(ec, bb, p);
		...
		if(hour%24==0) cout<<hour<<"hours:\t"<<n_24<<"\t"<<n_72<<"\t"<<n_192<<"\t"<<n_360<<"\t"<<n_672<<"\t"<<r_96<<"\t"<<r_168<<"\t"<<r_240<<"\t"<<r_336<<"\t"<<r_672<<endl;

	}

	//=====================3. compute least square========================
	double square=pow((n_24-d->n_24),2.0)+pow((n_72-d->n_72),2.0)+pow((n_192-d->n_192),2.0)+pow((n_360-d->n_360),2.0)+pow((n_672-d->n_672),2.0)+pow((r_96-d->r_96),2.0)+pow((r_168-d->r_168),2.0)+pow((r_240-d->r_240),2.0)+pow((r_336-d->r_336),2.0)+pow((r_672-d->r_672),2.0);
	cout<<"SD="<<square<<endl;
return square;
}
