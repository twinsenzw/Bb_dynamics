...
struct parameters
{
	...
	double Amax;
	double n;
	int Td; //amount of time for each antibody period
	int Nab; //total type of Ab activated for each antibody period.
...
public:
	...
	int phase; //antibody phase: 0=not activated, 1=activated, 2=activated in the last cycle, now depleting
};
...
void max_replace(vector< pair <int, double>> &maxbb, pair<int,double> testpair) //insert the testpair into the correct position of the maxbb, or discard testpair.
{
	for(int mbi=0;mbi<maxbb.size();mbi++)
	{
		if(maxbb[mbi].second < testpair.second)
		{
			pair<int,double> replaced_pair;
			replaced_pair.first=maxbb[mbi].first;
			replaced_pair.second=maxbb[mbi].second;
	
			maxbb[mbi].first=testpair.first;
			maxbb[mbi].second=testpair.second;
			
			testpair.first=replaced_pair.first;
			testpair.second=replaced_pair.second;
		}
	}
}
...
{
	if((t-p.tlag)%p.Td==0) //new cycle of antibody activation
	{
		//set all present activated abs to depleting, and present depleting abs to 0
		for(int ai=0;ai<ab.size();ai++)
		{
			if(ab[ai].phase==1) ab[ai].phase=2;
			else if(ab[ai].phase==2) {ab[ai].phase=0; ab[ai].concentration=0.0;}
		}

		//find the most abundant Nab types of Bb types, set the corresponding abs to 1
		vector< pair <int, double>> maxbb;
		
		for(int i=0;i<p.Nab;i++)
		{
			pair<int,double> newpair(-1,-1.0);
			maxbb.push_back(newpair);
		}

		for(int bi=0;bi<bb.size();bi++)
		{
			pair<int,double> testpair;
			testpair.first=bi;
			testpair.second=bb[bi].n_free+bb[bi].n_bound;
			max_replace(maxbb,testpair);
		}
		...
		for(int i=0;i<p.Nab;i++)
		{
			if(maxbb[i].first!=-1) 
			{
				ab[maxbb[i].first].phase=1;
				ab[maxbb[i].first].tinit=t;
			}
			cout<<maxbb[i].first<<";"<<maxbb[i].second<<"\t"; //sanity check:
		}
		cout<<endl;
	}
	...
	double present_period_conc=-1;
	for(int ai=0;ai<ab.size();ai++)
	{
		if(ab[ai].phase==0) ab[ai].concentration=0.000000001; //completely shutdown
		if(ab[ai].phase==1)
		{
			...
			present_period_conc=ab[ai].concentration;
		}	
	}

	for(int ai=0;ai<ab.size()&&present_period_conc!=-1;ai++)
	{
		if(ab[ai].phase==2) ab[ai].concentration=p.Amax-present_period_conc;
	}
}
...
void grow_EC(EC &ec, vector<BB> bb, parameters p) //for model 1, ec is a constant
{
...
double objective(const vector<double> &present_p, vector<double> &grad, void *data) 
{
	...
	p.Td=present_p[0]; // hours
	 //	
	p.p0=present_p[1]; //hours -1
	p.x=present_p[2];
	...
	cout<<p.Td<<"\t"<<p.Nab<<"\t"<<p.p0<<"\t"<<p.x<<endl; // hours-1

	for(int hour=1;hour<=360;hour++)
	{
		...
		if(hour%24==0) cout<<hour<<"hours:\t"<<n_24<<"\t"<<n_72<<"\t"<<n_192<<"\t"<<n_360<<"\t"<<r_96<<"\t"<<r_168<<"\t"<<r_240<<"\t"<<r_336<<endl;
...
int main()
{
 ...
 //=======fitting===========
	...
	nlopt::opt opt(nlopt::LN_COBYLA, 3); /* algorithm and dimensionality */

	opt.set_min_objective(objective, &data); 

	vector<double> present_p(3);
	//present_p[0]=0.9;
	present_p[0]=72; //Td
	//present_p[1]=2; //Nab
	present_p[1]=0.8; //p0
